...
import {
  useSharedDispatch,
  useSharedSelector,
} from "shared/utils/hooks/useShared";
...
export const ItemNav = <D,>({
  ...
  //  in relation to https://github.com/circuitPro/circuitPro/issues/579
  ...
  const [currentlyPressedObj, setCurPressedObj] = useState(
    undefined as Selectable | undefined
  );
  ...
  const deleteImg = useMemo(() => {
    ...
    if (!(currentlyPressedObj instanceof Component) || !hoveringNav) return;
  ...
  return (
    <>
      ...
      >
        ...
        {new Array(Clamp(numClicks - 1, 0, MAX_STACK - 1))
          .fill(0)
          .map((_, i) => (
            <div
          ...
          <div>
            {
              ...
              isEnabled && (
                <div
                  role="button"
                  tabIndex={0}
                  className={`itemnav__tab ${
                    isOpen ? "" : "itemnav__tab__closed"
                  }`}
                  title="Circuit Components"
                  onClick={() =>
                    dispatch(isOpen ? CloseItemNav() : OpenItemNav())
                  }
                >
                  <div></div>
                </div>
              ...
              <div>
                {section.items.map((item, j) => (
                  <div
                    ...
                    tabIndex={0}
                    onMouseEnter={() => {
                      item.removable && setHover(item.id);
                    }}
                    onMouseLeave={() => {
                      item.removable && setHover("");
                    }}
                  >
                    <Draggable
                    ...
                    >
                      <img
                        src={`/${config.imgRoot}/${section.id}/${item.icon}`}
                        alt={item.label}
                      />
                    </Draggable>
                    {item.removable && hovering === item.id && (
                      <div
                        ...
                        onClick={(ev) => {
                          ...
                          setCurItemImg("");
                          if (onDelete) onDelete(section, item);
                          setHover("");
