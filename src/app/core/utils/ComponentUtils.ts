...
import { GRID_SIZE, IO_PORT_LINE_WIDTH } from "./Constants";
...
export class IOObjectSet {
  ...
  public constructor(set: IOObject[] = []) {
    this.components = new Set<Component>(
      set.filter((o) => o instanceof Component) as Component[]
    );
...
export function CreateGroup(objects: IOObject[]): IOObjectSet {
  ...
  const wires = [
    ...
    (w) =>
      objs.includes(w.getP1Component()) && objs.includes(w.getP2Component())
  );
  ...
  while (queue.length > 0) {
    ...
    if (q instanceof Wire) {
      if (full) {
        ...
        if (isNode(p1) && !visited.has(p1)) queue.push(p1);

        const p2 = q.getP2Component();
        if (isNode(p2) && !visited.has(p2)) queue.push(p2);
      } else {
        const p2 = q.getP2Component();
        if (isNode(p2) && !visited.has(p2)) queue.push(p2);
      }
...
export function GetComponentPath(c: Component): Component[] {
  ...
  while (queue.length > 0) {
    ...
    for (const w of q.getConnections()) {
      if (!visited.has(w.getP1Component())) queue.push(w.getP1Component());
      if (!visited.has(w.getP2Component())) queue.push(w.getP2Component());
    }
  }
  ...
  return Serialize(objects, [
    {
      ...
      customBehavior: {
        ...
        customKeyFilter: (_: Port, key: string) => key !== "connections", // don't serialize connections (handle them above)
      },
    },
    {
      type: IOObject,
      customBehavior: {
        customKeyFilter: (_: IOObject, key: string) => key !== "designer", // don't serialize designer
      },
    },
  ]);
}
...
export function CopyGroup(objects: IOObject[]): IOObjectSet {
  if (objects.length === 0) return new IOObjectSet();
  ...
  //  See: https://github.com/circuitPro/circuitPro/issues/545
  for (const object of copies) {
    if (isPressable(object)) object.release();
  }
...
export function GetCameraFit(
  ...
  const worldSize = camera
    .getWorldPos(screenSize)
    .sub(camera.getWorldPos(V(0, 0))); // World size of camera view

  // Determine which bbox dimension will limit zoom level
  const ratio = V(
    bbox.getWidth() / worldSize.x,
    bbox.getHeight() / worldSize.y
  );
...
export function Snap(p: Vector): Vector {
  return V(
    Math.floor(p.x / GRID_SIZE + 0.5) * GRID_SIZE,
