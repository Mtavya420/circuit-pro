...
export const SelectionHandler: EventHandler = {
  ...
  getResponse: ({
    ...
    if (!input.isShiftKeyDown()) action.add(DeselectAll(selections));

    const ports = GetAllPorts(designer.getObjects());
    const objs = [
      ...designer.getObjects().reverse(),
      ...designer.getWires().reverse(),
    ];
    ...
    if (obj && !(hitPort && isWire)) {
      ...
      action.add(
        deselect ? Deselect(selections, obj) : Select(selections, obj)
      );
      action.add(Shift(designer, obj));
    }

    // https://github.com/circuitPro/circuitPro/issues/622
    if (!action.isEmpty()) history.add(action);
  },
};
