...
export function VersionConflictResolver(
  fileContents: string | Circuit
): string {
  const circuit =
    typeof fileContents === "string"
      ? (JSON.parse(fileContents) as Circuit)
      : fileContents;

  const v = parseFloat(circuit.metadata.version);

  const contents = JSON.parse(circuit.contents) as Record<
    string,
    SerializationEntry
  >;
  ...
  if (v < 2.1) {
    const transformations: Record<
      string,
      Array<{ ports: string; positioner: string }>
    > = {
      Multiplexer: [
        { ports: "inputs", positioner: "ConstantSpacePositioner" },
        { ports: "outputs", positioner: "Positioner" },
      ],
      Demultiplexer: [
        { ports: "outputs", positioner: "ConstantSpacePositioner" },
  ...
  //  https://github.com/circuitPro/circuitPro/pull/1087
  if (v < 3.1) {
    ...
    const transformations = {
      ...
      LED: [{ prevKey: "color", newKey: "color", defaultVal: "#ffffff" }],
      Oscilloscope: [
        { prevKey: "frequency", newKey: "delay", defaultVal: 0 },
      ...
      string,
      Array<{ prevKey: string; newKey: string; defaultVal?: unknown }>
    >;

    Object.values(contents).forEach(({ type, data }) => {
      const transformation = transformations[type] ?? [];

      if (transformation.length === 0) return;
  ...
  if (v < 3.2) {
    ...
    const getEntry = (parent: SerializationEntry, key: string) => {
      ...
      if (!v) return;
      if (isRef(v)) return contents[v["ref"]];
    ...
    Object.values(contents).forEach((entry) => {
      ...
      entry.data["props"] = {
        type: "",
        data: {
          ...((entry.data["props"] as SerializationEntry) ?? { data: {} })[
            "data"
          ],
          pos: getEntry(t, "pos")!,
...
export function VersionConflictPostResolver(
  version: string,
  data: ContentsData
) {
  ...
  if (v < 3) {
    ...
    designer
      ...
      .forEach((ic: IC) => {
        ...
        const wrongInputs = inputs.filter(
          (i) => !INPUT_WHITELIST.some((type) => i instanceof type)
        );
