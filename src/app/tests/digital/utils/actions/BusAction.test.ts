...
describe("Bus Action", () => {
  ...
  test("Simple 2-port connection", () => {
    ...
    expectBusConnections(
      [i1.getOutputPort(0), i2.getOutputPort(0)],
      g1.getInputPorts()
    );
  ...
  test("Rotated Constant Number -> BCD 4-port", () => {
    ...
    // From PR #1020 (https://github.com/circuitPro/circuitPro/pull/1020#pullrequestreview-914672445)
    i1.setPos(V(-2, 2));
  ...
  test("Multiple Rotated Switches -> BCD 4-port", () => {
    ...
    expectBusConnections(
      [i1, i2, i3, i4].map((i) => i.getOutputPort(0)),
      o1.getInputPorts()
    );
  ...
  test("Multiple Rotated Switches -> Mux 6-port", () => {
    const [i1, i2, i3, i4, i5, i6, o1] = Place(
      new Switch(),
      new Switch(),
      new Switch(),
      new Switch(),
    ...
    expectBusConnections(
      [i1, i2, i3, i4, i5, i6].map((i) => i.getOutputPort(0)),
  ...
  test("Multiple Rotated Switches + Constant Number -> Mux 6-port", () => {
    ...
    expectBusConnections(
      [i1, i2, i3].flatMap((i) => i.getOutputPorts()),
      [...o1.getInputPorts(), ...o1.getSelectPorts()]
    );
  ...
  test("Simple IC 4-port", () => {
    ...
    const icdata = ICData.Create([i1, i2, i3, i4]);
    {
      ...
      const setPort = (p: number, pos: Vector, dir: Vector) => {
        ...
        icdata!
          .getPorts()
          [p].setTargetPos(pos.add(dir.normalize().scale(IO_PORT_LENGTH)));
      };
    ...
    expectBusConnections(
      [i1, i2, i3, i4].flatMap((i) => i.getOutputPorts()),
      ic.getInputPorts()
    );
  ...
  describe("Bus Components", () => {
    ...
    test("6 Switches -> Mux", () => {
      const [i1, i2, i3, i4, i5, i6, m1] = Place(
        new Switch(),
        new Switch(),
        new Switch(),
        new Switch(),
      ...
      const [iports, oports] = GetComponentBusPorts([
        i1,
        i2,
        i3,
        i4,
    ...
    test("Fail: 7 Switches -> Mux", () => {
      const [i1, i2, i3, i4, i5, i6, i7, m1] = Place(
        new Switch(),
        new Switch(),
        new Switch(),
        new Switch(),
