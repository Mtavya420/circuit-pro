...
describe("Delete Group Action", () => {
  ...
  // See https://github.com/circuitPro/circuitPro/issues/902
  describe("Deletion of parts of Tree", () => {
    function PlaceTree() {
      const leds = new Array(8).fill(0).map((_) => Place(new LED())[0]);
      const nodes2 = new Array(4)
        .fill(0)
        .map((_) => Place(new DigitalNode())[0]);
      const nodes1 = new Array(2)
        .fill(0)
        .map((_) => Place(new DigitalNode())[0]);
      ...
      nodes2.forEach((n, i) =>
        n.setPos(
          V(3, (leds[2 * i].getPos().y + leds[2 * i + 1].getPos().y) / 2)
        )
      );
      ...
      nodes1.forEach((n, i) =>
        n.setPos(
          V(2, (nodes2[2 * i].getPos().y + nodes2[2 * i + 1].getPos().y) / 2)
        )
      );
    ...
    function expectTreePlacement([sw, nodes1, nodes2, leds]: ReturnType<
      typeof PlaceTree
    >) {
    ...
    test("Delete Switch", () => {
      ...
      [sw, ...nodes1, ...nodes2].forEach((o) =>
        expect(designer.getObjects()).not.toContain(o)
      );
